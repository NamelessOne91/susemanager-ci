...
variable "URL_PREFIX" {
  type = string
  default = "https://ci.suse.de/view/Manager/view/Manager-5.2/job/manager-5.2-qe-build-validation-paygo-aws/"
}
...
// change to spacewalk after public release
variable "CUCUMBER_GITREPO" {
  type = string
  default = "https://github.com/uyuni-project/uyuni.git"
}

// change to Manager-5.2 after public release
...
variable "MAIL_SUBJECT" {
  type = string
  default = "Results MManager5.2-AWS-BV $status: $tests scenarios ($failures failed, $errors errors, $skipped skipped, $passed passed)"
}
...
module "base" {
  ...
  ssh_key_path             = "~/.ssh/id_ed25519.pub.worker"
  provider_settings        = {
...
module "server" {
  ...
  image                      = var.SERVER_AMI != "" ? var.SERVER_AMI : "smlm-server-52-${var.ARCHITECTURE}-ltd-paygo"
  main_disk_size             = 100
